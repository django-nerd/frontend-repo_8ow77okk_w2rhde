      ...
      <div className="mt-10 grid md:grid-cols-2 gap-6">
        {products.map(p => (
          <article key={p.id} className="p-6 rounded-3xl bg-white border border-emerald-100 flex gap-6 items-center">
            <img src={p.image_url || 'https://images.unsplash.com/photo-1524594227082-cd1f9f3580f4?q=80&w=800&auto=format&fit=crop'} alt="product" className="w-40 h-40 object-cover rounded-2xl"/>
            <div className="flex-1">
              <h3 className="text-xl font-semibold text-emerald-900">{p.title}</h3>
              <p className="text-sm text-slate-600 mt-1">{p.description}</p>
              <div className="mt-4 flex items-center justify-between">
                <div className="text-emerald-800 font-semibold">${p.price.toFixed(2)}</div>
